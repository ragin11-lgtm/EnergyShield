...
The README's local run command binds the dashboard to `127.0.0.1`. The checked-in configuration does not force a local-only address because managed hosting platforms provide their own server binding. The V1 dashboard has no application-level authentication and should only be shared through an appropriate managed hosting service.
